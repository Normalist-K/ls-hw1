...
def train(audio_model, train_loader, test_loader, args, infer_loader):
    ...
    if args.dataset == 'audioset':
        ...
        print('scheduler for med is used')
        scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, list(range(5,26)), gamma=0.5)
        main_metrics = 'acc'
        loss_fn = nn.CrossEntropyLoss()
        warmup = True
    else:
    ...
    while epoch < args.n_epochs + 1:
        ...
        stats, valid_loss = validate(audio_model, test_loader, args, epoch, infer=False)
        _stats, _valid_loss = validate(audio_model, infer_loader, args, epoch, infer=True)
...
def validate(audio_model, val_loader, args, epoch, infer=False):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    batch_time = AverageMeter()
    if not isinstance(audio_model, nn.DataParallel):
        audio_model = nn.DataParallel(audio_model)
    ...
    with torch.no_grad():
        ...
        dir = args.exp_dir
        if infer == False:
            if os.path.exists(dir+'/predictions') == False:
                os.mkdir(dir+'/predictions')
                np.savetxt(dir+'/predictions/target.csv', target, delimiter=',')
            np.savetxt(dir+'/predictions/predictions_' + str(epoch) + '.csv', audio_output, delimiter=',')
        elif infer == True:
            if os.path.exists(dir+'/inference') == False:
                os.mkdir(dir+'/inference')
                np.savetxt(dir+'/inference/target.csv', target, delimiter=',')
            np.savetxt(dir+'/inference/predictions_' + str(epoch) + '.csv', audio_output, delimiter=',')
...
def validate_ensemble(args, epoch):
    ...
    ######################################
    target = np.loadtxt(exp_dir+'/inference/target.csv', delimiter=',')
    if epoch == 1:
        cum_predictions = np.loadtxt(exp_dir + '/inference/predictions_1.csv', delimiter=',')
    else:
        cum_predictions = np.loadtxt(exp_dir + '/inference/cum_predictions.csv', delimiter=',') * (epoch - 1)
        predictions = np.loadtxt(exp_dir+'/inference/predictions_' + str(epoch) + '.csv', delimiter=',')
        cum_predictions = cum_predictions + predictions
        # remove the prediction file to save storage space
        os.remove(exp_dir+'/inference/predictions_' + str(epoch-1) + '.csv')

    cum_predictions = cum_predictions / epoch
    np.savetxt(exp_dir+'/inference/cum_predictions.csv', cum_predictions, delimiter=',')
    #######################################
